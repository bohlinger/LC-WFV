...
parser = argparse.ArgumentParser(
    description="""
create LC-WFV grib2 file

Usage:
./create_grib2.py -d 2019111906
    """,
    formatter_class = RawTextHelpFormatter
    )
parser.add_argument("-d", metavar='datestr',
    help="string of date and time YearMonthDayHour")

args = parser.parse_args()

now = datetime(int(args.d[0:4]), int(args.d[4:6]), int(args.d[6:8]), int(args.d[8:10]))
init_fname = now.strftime("%Y%m%dT%H")
outpath = "/lustre/storeA/project/fou/om/LC-WFV/"
